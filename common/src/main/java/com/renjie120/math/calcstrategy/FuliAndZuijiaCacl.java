...
public class FuliAndZuijiaCacl implements ICaclSumMethod { 
	...
	private double[] calcSum(CalcInput input) {
		...
		// 全部的本金
		double allCost = input.getBen();
		// 全部的金额（本金+收益）
		double allMoney = input.getBen();
		// 计算月化收益
		double month_fee = MathTool.divide(input.getFee(), 12, 4);
		// 循环期数为月份数.
		for (int i = 0; i < input.getMonth(); i++) {
			if (i > 0) {
				allCost = MathTool.add(allCost, input.getAddon());
				allMoney = MathTool.add(allMoney, input.getAddon());
			}
	...
	public List<ComeInPlan> plan(CalcInput input) {
		...
		for (int i = 0; i < input.getMonth(); i++) {
			...
			lastBen = MathTool.add(p1.endMoney, input.getAddon());

			// 下面计算的是全部的投资的本金
			if (i > 0) {
				costBen = MathTool.add(costBen, input.getAddon());
			} 
			result.add(p1);
		}
		return result;
