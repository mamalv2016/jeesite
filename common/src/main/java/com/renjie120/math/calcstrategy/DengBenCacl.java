...
public class DengBenCacl implements ICaclSumMethod {  
	...
	private void checkArgument(CalcInput input) {
		Preconditions.checkArgument(input.getMonth() >= 0, "月份必须大于0");
	}
	...
	public double lixi(CalcInput input) {
		...
		// 计算月化收益
		double month_fee = MathTool.divide(input.getFee(), 12, 4);
		double temp3 = MathTool.add(MathTool.divide(input.getBen(), input.getMonth(), 5),
				MathTool.multiply(input.getBen(), month_fee));
		double temp4 = MathTool.multiply(
				MathTool.divide(input.getBen(), input.getMonth(), 5), 1 + month_fee);
		double temp5 = MathTool.add(temp3, temp4);
		return MathTool.subtract(
				MathTool.multiply(MathTool.divide(temp5, 2, 5), input.getMonth()),
				input.getBen());
	}

	@Override
	public boolean accept(CalcInput input) {
		return input.getCalcMethod() == 3;
	}
	...
	public List<BackPlan> plan(CalcInput input) {
		...
		double allBack = 0;
		// 每期偿还的本金
		double everyCostBen = MathTool.divide(input.getBen(), input.getMonth(), 5);
		for (int i = 0; i < input.getMonth(); i++) {
			...
			// 累计还款比率
			p1.rate = MathTool.divide(p1.allBack, input.getBen(), 3);
